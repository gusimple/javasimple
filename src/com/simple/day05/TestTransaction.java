...
public class TestTransaction {
	...
	@Test
	public void tes1(){
		Stream<Transaction> sorted = transactions.stream().filter(e -> e.getYear() == 2011).sorted((e1, e2)->Integer.compare(e1.getValue(),e2.getValue()));
		...
		Stream<Trader> cambridge = transactions.stream()
				...
				.sorted((e2, e3) -> e2.getName().compareTo(e3.getName()))
				.distinct();
		cambridge.forEach(System.out::println);

		System.out.println("返回所有交易员姓名字符串,按字母顺序排序");
		...
		Stream<String> sorted1 = transactions.stream()
				.map(e -> e.getTrader().getName())
				.flatMap(TestTransaction::filterCharacter1)
				.sorted((e1, e2) -> e1.compareToIgnoreCase(e2));
		sorted1.forEach(System.out::print);
		System.out.println("有没有在米兰工作的");
		boolean milan = transactions.stream().noneMatch(e -> e.getTrader().getCity().equals("Milan"));
		System.out.println(milan);
		System.out.println("打印总交易额");
		Optional<Integer> reduce1 = transactions.stream()
				.filter(e1->e1.getTrader().getCity().equals("Cambridge"))
				.map(Transaction::getValue)
				.reduce(Integer::sum);
		System.out.println(reduce1.get());
		System.out.println("所有交易中最高的");
